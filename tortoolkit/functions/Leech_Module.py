...
async def check_link(msg, rclone=False, is_zip=False, extract=False, prev_msg=None):
    ...
    if extract:
        mess = f"You chose to extract the archive <a href='tg://user?id={omess.sender_id}'>єитєя ραѕѕωσя∂ (ιf αиу).</a>\n υѕє <code>/setpass {omess.id} ραѕѕωσя∂-нєяє</code>"
    ...
    if msg is None:
        ...
        if name is None:
            await omess.reply(
                "тнιѕ ιѕ иσт α тσяяєит fιℓє тσ ℓєє¢н fяσм. ѕєи∂ <code>.torrent</code> fιℓє",
                parse_mode="html",
            )
        elif name.lower().endswith(".torrent"):
            rmess = await omess.reply("∂σωиℓσα∂ιиg тнє тσяяєит fιℓє.")
            ...
            if not isinstance(torrent_return, bool) and torrent_return is not None:
                ...
                if not rclone:
                    ...
                    if res is None:
                        await msg.reply(
                            "<b>υρℓσα∂ тσ ∂яινє νια я¢ℓσиє нαѕ fαιℓє∂. ρℓєαѕє ¢нє¢к вσт ℓσgѕ</b>",
                            parse_mode="html",
                        )
                    await QBittorrentWrap.delete_this(dl_task.hash)
    ...
    while True:
        ...
        if isinstance(ext_path, str):
            if "Wrong Password" in ext_path:
                mess = f"<a href='tg://user?id={omess.sender_id}'>яє-єитєя ραѕѕωσя∂</a>\nтнє ραѕѕωσя∂ <code>{password}</code> уσυ ρяσνι∂є∂ ιѕ α ωяσиg ραѕѕωσя∂.уσυ нανє {((time.time()-start)/60)-20} мιиѕ тσ яєρℓу єℓѕє υи єχтяα¢тє∂ zιρ ωιℓℓ вє υρℓσα∂є∂.\n Use <code>/setpass {omess.id} password-here</code>"
                await omess.reply(mess, parse_mode="html")
                wrong_pwd = True
            else:
                await clear_stuff(path)
