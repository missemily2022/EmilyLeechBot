...
async def add_magnet(aria_instance, magnetic_link, c_file_name):
    try:
        ...
        return (
            False,
            "**FAILED** \n" + str(e) + " \nρℓєαѕє ∂σит ѕєи∂ ѕℓσω ℓιикѕ.яєα∂ /help",
        )
    else:
...
async def add_torrent(aria_instance, torrent_file_path):
    if torrent_file_path is None:
        return (
            False,
            "**FAILED** \n\nѕσмєтнιиg ωєит ωяσиg ωнєи тяуιиg тσ α∂∂ <u>TORRENT</u> fιℓє",
        )
    if os.path.exists(torrent_file_path):
        ...
        return (
            ...
            "**FAILED** \n"
            + str(e)
            + " \nρℓєαѕє тяу σтнєя ѕσυя¢єѕ тσ gєт ωσякαвℓє ℓιик",
        )
...
async def aria_dl(incoming_link, c_file_name, sent_message_to_update_tg_p, user_msg):
    ...
    if incoming_link.startswith("magnet:"):
        ...
        if err_message is not None:
            ...
            await ar_task.set_inactive("¢αи'т gєт мєтα∂αтα.\n")
            return False, ar_task
    await asyncio.sleep(1)

    if op is None:
        await ar_task.set_inactive(
            "киσωи єяяσя. иσтнιиg ωяσиg нєяє. уσυ ∂ι∂и'т fσℓℓσω ιиѕтяυ¢тισиѕ."
        )
        return False, ar_task
    else:
        statusr, stmsg = op
...
async def check_progress_for_dl(
    ...
    try:
        ...
        if not complete:
            ...
            await event.edit(
                f"**∂σωиℓσα∂ ¢σмρℓєтє∂:** `{file.name}`\n\n**ѕιzє:** `{file.total_length_string()}`",
                buttons=None,
            )
            return True, "**∂σωиℓσα∂ ¢σмρℓєтє**"
        ...
        if " not found" in str(e) or "'file'" in str(e):
            ...
            return False, f"тнє ∂σωиℓσα∂ ωαѕ ¢αи¢єℓє∂. {fname}"
        else:
        ...
        return False, "тнє ℓιик ιѕ вαѕι¢αℓℓу ∂єα∂."
    except Exception as e:
        torlog.info(str(e))
        if " not found" in str(e) or "'file'" in str(e):
            return False, "тнє ∂σωиℓσα∂ ωαѕ ¢αи¢єℓє∂."
        else:
